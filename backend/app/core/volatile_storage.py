...
async def save_temp_file(
    ...
    target_dir = TEMP_DIR / sub_dir if sub_dir else TEMP_DIR
    ...
    for meta_path in TEMP_DIR.rglob("*.meta.json"):
        ...
        if meta.get("file_id") == file_id:
            ...
            if _is_expired(meta):
                ...
                _delete_file_and_meta(meta_path)
                return None

            # Obtener ruta del archivo de datos
            data_path = meta_path.with_suffix("")  # Remover .json
            # El archivo de datos tiene el nombre original sin .meta.json
            # meta_path = file.ext.meta.json, data_path = file.ext
...
def _delete_file_and_meta(meta_path: Path) -> None:
    """Elimina un archivo de datos y su archivo de metadatos."""
    data_path = Path(str(meta_path).replace(".meta.json", ""))
    try:
        if data_path.exists():
            data_path.unlink()
